...
async def get_all_statistics(supply_repo) -> dict[str, SupplyStatistics]:
    ...
    for supply_id in supply_repo._supplies.keys():
        ...
        for bidder_id in bidder_ids:
            wins = await redis_client.get(f"stats:{supply_id}:bidder:{bidder_id}:wins") or 0
            revenue = await redis_client.get(f"stats:{supply_id}:bidder:{bidder_id}:revenue") or 0.0
            no_bids = await redis_client.get(f"stats:{supply_id}:bidder:{bidder_id}:no_bids") or 0
